...
// Buffer para copia temporal de formatos en FRTOS_snprintf_P.
static char lbuf[PRINTF_BUFFER_SIZE];
...
int FRTOS_snprintf_P(char* buffer, size_t count, const char* format, ...)
{
  ...
  while ( xSemaphoreTake( sem_PRINTF, ( TickType_t ) 1 ) != pdTRUE )
		taskYIELD();

  // Copio el formato en PSTR a un buffer local en RAM que es donde _vsnprintf lo lee.
  memset(&lbuf, '\0', PRINTF_BUFFER_SIZE );
  strcpy_P(lbuf,format);

  //va_start(va, format);
  va_start(va, lbuf);
  //int ret = _vsnprintf(buffer, count, format, va);
  int ret = _vsnprintf(buffer, count, lbuf, va);
  va_end(va);

  xSemaphoreGive( sem_PRINTF );
...
{
	sem_PRINTF = xSemaphoreCreateMutex();
}
